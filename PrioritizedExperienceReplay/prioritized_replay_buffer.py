...
def prioritized_replay_buffer(
		...
		# parameters of prioritized replay buffer
		alpha: float = 0.2,
		beta: float = 0.6,
	...
	def compute_loss(data: Dict[str, Tensor]) -> Tuple[Tensor, Dict[str, float]]:
		...
		# Note:
		# Since we need to do importance sampling before averaging,
		# we shouldn't execute torch.mean() here.
		# So, we return element-wise loss.
		elementwise_loss = (action_value-td_target)**2
		
		information = dict(
			Action_Value=action_value.detach().cpu().numpy(),
		)
		return elementwise_loss, information
	...
	def update(step):
		data = replay_buffer.sample_batch(device, beta=beta)
		weights = torch.squeeze(data['weights'])
		
		# Importance sampling before averaging.
		elementwise_loss, information = compute_loss(data)
		...
		loss_for_priority = elementwise_loss.detach().cpu().numpy()
		new_priorities = loss_for_priority + priority_eps
