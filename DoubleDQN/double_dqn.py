...
def double_dqn(
	...
	# Instantiate training and testing environment.
	env, test_env = make_env(), make_env()
	
	# Instantiate agent.
	agent = DoubleDQNAgent(env.observation_space, env.action_space).to(device)
	...
	logger.log("Number of variables: \t value function: %d\t" % num_variables)
	...
	def compute_loss(data: Dict) -> Tuple[Tensor, Dict]:
		...
		# next_action is not selected by target agent in Double DQN,
		# in order to decouple action selection and policy evaluation.
		with torch.no_grad():
			next_action = torch.argmax(agent(next_observation), dim=1, keepdim=True)
			next_action_value = torch.squeeze(
				target_agent(next_observation, next_action)
			)
	...
	def update(step):
		...
		# Update target agent every target_update steps.
...
def main():
	...
	for seed in seeds:
		...
		double_dqn(lambda: gym.make(env_name), seed, logger_kwargs)
