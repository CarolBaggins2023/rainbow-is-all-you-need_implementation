...
class DuelingNetwork(nn.Module):
	def __init__(self, in_dim: int, out_dim: int, out_activation: Callable):
		super().__init__()
	...
	def forward(self, x: Tensor) -> Tensor:
		...
		x_advantage = self.fc_advantage(x)
		
		x_action = self.out_activation(
			x_state + x_advantage - torch.mean(
				x_advantage, dim=-1, keepdim=True
			)
		)
		return x_action
...
class DuelingDQNAgent(nn.Module):
	def __init__(
		...
		self.value_function = DuelingNetwork(
